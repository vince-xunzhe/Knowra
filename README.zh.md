...
应用左侧导航分 5 个页面（`知识 / 论文 / 回顾 / 看板 / 设置`），每页都是一个独立工作区；跨页的状态（LLM 调用埋点、Wiki 编译进度、论文处理状态）通过同一份后端串联。

### 知识 · Knowledge Graph

主工作台。把"目录里的一堆 PDF"逐步变成"被精选过的、可检索的 wiki 层"，并基于编译后的知识层提供图谱、Ask、健康检查等所有运维入口。

![Knowra 知识页 · 默认视图](docs/screenshots/01-graph-default.png)

- **流水线控制台（左侧）** — 4 个 stage 卡片（`① 录入 / ② 筛选 / ③ 编译 / ④ 健检`）依次展开，每张卡有自己的状态徽章（idle / running / warning / ok），系统建议的下一步会发光，点击展开能看到该阶段的全部按钮（扫描+处理、自动剔除、重编译、健康检查等）。

  ![流水线控制台展开](docs/screenshots/02-graph-pipeline-console.png)

- **Ask 抽屉** — 跨 wiki 的问答 agent。agent 调用 `list_wiki_index / search_wiki / read_wiki` 工具对编译后的 `.md` 层检索，返回带引用的 markdown 答案，可查看 tool trace，支持把好答案存为概念页或导出为 Marp 幻灯片 / 结构化报告。

  ![Ask 抽屉对话](docs/screenshots/03-graph-ask-drawer.png)

- **Wiki 健康检查** — 规则层 + 单次 agent 调用产出 `待充实 / 可合并 / 待建概念`，再补 5 条追问。每条都有内联操作（重编译 / 标记为可接受 / 合并 / 追问 → Ask）。

  ![健康检查弹窗](docs/screenshots/04-graph-lint-modal.png)

### 论文 · Papers

论文库。扫描配置好的 PDF 目录、显示每篇论文的处理状态、批量处理或重处理失败论文、暴露失败摘要。

![论文库页面](docs/screenshots/05-papers.png)

### 回顾 · Review

单篇论文精读工作区。展示结构化抽取（研究问题 / 动机 / 技术 / 数据集 / 核心贡献 / 关键发现 / 关键公式）、个人笔记侧栏、单篇追问对话框，以及首页预览。

![回顾页 · 默认视图](docs/screenshots/06-review-default.png)

- **应用内 PDF 浮窗** — 点首页预览右上角的 `展开 PDF` 按钮，PDF 阅读器从右侧滑入。左侧论文列表自动折叠为零宽，结构化抽取保持可见，PDF 浮窗覆盖 `个人笔记 + 首页预览` 区域但**不遮挡核心贡献列**。支持按钮 / 键盘缩放（`+ / − / 0`）、`Ctrl/⌘ + 滚轮` 缩放，**跨缩放保留位置**：视口锚定在同一段落，不会因放大缩小回到顶部。关闭（Esc / X / 点外部）会把当前 paper 的滚动位置 + 缩放比例存下来，下次打开同一篇时从原地继续读。

  ![回顾页 · PDF 浮窗打开](docs/screenshots/07-review-pdf-open.png)

### 看板 · Dashboard

只读的数据看板，描绘整个知识库当前的形状。后端一次性聚合所有指标，每个 widget 都基于同一份快照。

- **Top-6 知识方向雷达** — 取高频标签前 6 个为轴，叠加 3 条系列（论文 / 概念 / 边密度，全部归一化）。
- **增长曲线** — 最近 12 周的论文 / 概念 / 边数量时间序列，源自现有时间戳，无需快照表。
- **分布饼图 + 标签云** — `论文分类` 和 `节点类型` 饼图带颜色 chip legend，标签云 Top-20。
- **策展健康** — `status × promoted_by` 堆叠柱图 + 最早 pending 候选的积压天数。
- **网络结构** — 度排序的中枢概念 Top-10、孤儿节点数、平均度、关系类型分布。
- **编译与健检状态** + **30 天 LLM 使用** — 调用数、Token、平均延迟，按任务 + 按模型双视图。

![看板页](docs/screenshots/08-dashboard.png)

### 设置 · Settings

任务路由模型网关。按逻辑任务（`paper_extract / paper_chat / embedding / wiki_compile / ask_agent / ask_synthesis / promotion_judge / wiki_lint`）绑定不同模型，可在已配置的 provider（OpenAI / OpenAI 兼容 / 本地 Codex CLI）中切换、做联通测试，并调相似度阈值 + 维护操作。

![设置页](docs/screenshots/09-settings.png)
...
原生启动时，`bash start.sh` 默认关闭后端热重载，避免修改源码中断模型任务。开发时可用 `KNOWLEDGE_WIKI_MODE=native BACKEND_RELOAD=1 bash start.sh` 显式开启。论文处理不再挂在 HTTP 响应后台任务中；重启仍会中断尚未完成的工作，需要重新执行，已保存的结果会保留。

本地模式下，同一数据目录只能启动一个后端，换端口也不能重复启动。SQLite 使用 WAL，锁等待时间为 30 秒；论文处理遇到临时锁冲突会先回滚再有限重试，复用本轮已成功的模型结果。备份运行中的数据库应使用 SQLite backup API（迁移和去重脚本已使用），不要只复制 `knowledge.db`，因为已提交的数据可能仍在 WAL 附属文件中。
