  ...
  return (
    <aside role="status" aria-live="polite" className="fixed bottom-5 left-24 z-30 w-72 rounded-xl border border-slate-700 bg-slate-900/95 p-3 shadow-xl backdrop-blur-md">
      <div className="flex items-center gap-2 text-sm font-medium text-slate-100">
        ...
        <span>{running ? tr("健康检查进行中") : job.phase}</span>
        {!running && <button aria-label={tr("关闭健康检查提醒")} className="ml-auto text-slate-400" onClick={() => dismissNotification(jobId)}><X size={14} /></button>}
      </div>
      <p className="mt-1 text-xs text-slate-400">{connectionError ? tr("状态连接暂时中断，正在重连…") : running ? tr("{0}，可继续浏览。", { 0: job.phase }) : warning ? tr("请查看报告或重新运行检查。") : tr("报告已保存，可查看发现的问题。")}</p>
      <button onClick={() => { if (!running) dismissNotification(jobId); onOpen() }} className="mt-2 text-xs text-indigo-300 hover:text-indigo-200">{running ? tr("查看状态") : tr("查看报告")}</button>
    </aside>
  )
}
