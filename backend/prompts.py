...
- principle.key_formulas: 优先列 2-4 条论文真正出现且最关键的公式；每条 {name: "式(3) 自注意力" 之类, formula: "仅填写 KaTeX 兼容的标准 LaTeX", plain: "解释变量、输入输出以及这条公式解决什么问题"}。不得把普通描述伪造成公式；论文少于 2 条关键公式时按实际数量返回，没有公式则返回 []
    · formula 不包含 `$` / `$$` / `\\[` / `\\]` 外层定界符；`{}` 只用于 LaTeX 分组，需要显示集合花括号时写 `\\{` 和 `\\}`
    · 使用 `\\sim`、`\\sum`、`\\theta`、`\\hat{y}`、`\\mathbb{E}`、`\\mid`、`\\|` 等标准命令，不得混用 `～`、`Σ`、`θ`、`ŷ`、`E_ŷ~_{...}` 等 Unicode/伪 LaTeX 写法
    · formula 位于 JSON 字符串中，输出文本里的 LaTeX 反斜杠必须按 JSON 规则转义；例如解析后的 `\\theta` 在 JSON 源文本中应写为 `\\\\theta`
